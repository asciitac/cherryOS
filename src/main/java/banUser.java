...
public class banUser extends JPanel {
    ...
    public banUser(DiscordApi api, Server server) throws InterruptedException {
        ...
        banButton.addActionListener(actionEvent -> {
            ...
            server.banUser(user, 0, reasonField.getText());
            feedback.setText(user.getDiscriminatedName() + " banned successfully!");
            try {
    ...
    public static void main (DiscordApi api, Server server) throws InterruptedException {
        JFrame frame = new JFrame ("Ban User");
        frame.setDefaultCloseOperation (JFrame.HIDE_ON_CLOSE);
        frame.getContentPane().add (new banUser(api, server));
        frame.pack();
